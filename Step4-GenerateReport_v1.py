        ...
        if missing_images:
            for entry in missing_images:
                ...
                for filename in os.listdir(foldername):
                    file_info = extract_file_info_v5(os.path.join(foldername, filename))
                    if file_info is not None and file_info["image_position_var"] == entry["Image Position"]:
                        missing_entry["Aspect Ratio"] = file_info["aspect_ratio"]
                        break
...
with open(report_file, mode='w', newline='') as csvfile:
    fieldnames = ["Subfolder", "Material", "Frame", "Image Position", "Aspect Ratio"]    
